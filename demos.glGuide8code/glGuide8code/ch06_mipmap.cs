...
namespace demos.glGuide8code {
    ...
    public unsafe class ch06_mipmap : _glGuide8code {
        ~ch06_mipmap() {
        ...
        public ch06_mipmap(Form mainForm, int width, int height, GL gl)
        ...
        public override void init(CSharpGL.GL gl) {
            {
                var vsCodeFile = "06/ch06_mipmap/skybox_shader.vs.glsl";
                var fsCodeFile = "06/ch06_mipmap/skybox_shader.fs.glsl";
            ...
            }
            {
                ...
                gl.glBufferData(GL.GL_ARRAY_BUFFER,
                    sizeof(GLfloat) * (plane_vertices.Length + plane_texcoords.Length), IntPtr.Zero, GL.GL_STATIC_DRAW);
                fixed (GLfloat* p = plane_vertices) {
                    gl.glBufferSubData(GL.GL_ARRAY_BUFFER,
                        0, sizeof(GLfloat) * plane_vertices.Length, (IntPtr)p);
                }
                fixed (GLfloat* p = plane_texcoords) {
                    gl.glBufferSubData(GL.GL_ARRAY_BUFFER,
                        sizeof(GLfloat) * plane_vertices.Length, sizeof(GLfloat) * plane_texcoords.Length, (IntPtr)p);
                }
                ...
                gl.glVertexAttribPointer(1, 2, GL.GL_FLOAT, false, 0, (IntPtr)(sizeof(GLfloat) * plane_vertices.Length));
            ...
            }
            {
                ...
                uint[] colors = { 0xFF0000FF, 0xFF00FF00, 0xFFFF0000, 0xFF00FFFF, 0xFFFF00FF, 0xFFFFFF00, 0xFFFFFFFF };
                for (var i = 0; i < 7; i++) {
                    for (var j = 0; j < 64 * 64; j++) {
                        data[j] = colors[i];
                    }
                    fixed (uint* p = data) {
