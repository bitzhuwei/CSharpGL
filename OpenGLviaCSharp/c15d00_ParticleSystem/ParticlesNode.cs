        ...
        {
            {
                ...
                //var array = (vec4*)this.attactorBuffer.MapBuffer(MapBufferAccess.ReadWrite);
                //for (int i = 0; i < attractorCount; i++)
                //{
                //    array[i] = new vec4(
                //        (float)(Math.Sin(attractorInterval * (i + 4) * 7.5f * 20.0f)) * 50.0f,
                //        (float)(Math.Cos(attractorInterval * (i + 7) * 3.9f * 20.0f)) * 50.0f,
                //        (float)(Math.Sin(attractorInterval * (i + 3) * 5.3f * 20.0f))
                //            * (float)(Math.Cos(attractorInterval * (i + 5) * 9.1f)) * 100.0f,
                //        array[i].w);
                //    //array[i] = new vec4(1, 1, 1, 1) * 5;
                //}
                //this.attactorBuffer.UnmapBuffer();
            }
            {
                ...
                glBindImageTexture(imageUnit0, this.texVelocity.Id, level, layered, layer, GL.GL_READ_WRITE, GL.GL_RGBA32F);
                glBindImageTexture(imageUnit1, this.texPosition.Id, level, layered, layer, GL.GL_READ_WRITE, GL.GL_RGBA32F);
                this.computeProgram.SetUniform("dt", deltaTime);
