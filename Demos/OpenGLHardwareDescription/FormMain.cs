...
namespace OpenGLHardwareDescription
{
    public partial class FormMain : Form
    {
        ...
        private string GetOpenGLInfo()
        {
            ...
            GL.Instance.GetIntegerv(GL.GL_MAX_VERTEX_IMAGE_UNIFORMS, maxImageUniforms);
            builder.AppendLine(string.Format("Max Vertex Image Uniforms: {0}", maxImageUniforms[0]));
            GL.Instance.GetIntegerv(GL.GL_MAX_TESS_CONTROL_IMAGE_UNIFORMS, maxImageUniforms);
            builder.AppendLine(string.Format("Max TESS CONTROL Image Uniforms: {0}", maxImageUniforms[0]));
            GL.Instance.GetIntegerv(GL.GL_MAX_TESS_EVALUATION_IMAGE_UNIFORMS, maxImageUniforms);
            builder.AppendLine(string.Format("Max TESS EVALUATION Image Uniforms: {0}", maxImageUniforms[0]));
            GL.Instance.GetIntegerv(GL.GL_MAX_GEOMETRY_IMAGE_UNIFORMS, maxImageUniforms);
            builder.AppendLine(string.Format("Max Geometry Image Uniforms: {0}", maxImageUniforms[0]));
            GL.Instance.GetIntegerv(GL.GL_MAX_FRAGMENT_IMAGE_UNIFORMS, maxImageUniforms);
            builder.AppendLine(string.Format("Max Fragment Image Uniforms: {0}", maxImageUniforms[0]));
            GL.Instance.GetIntegerv(GL.GL_MAX_COMBINED_IMAGE_UNIFORMS, maxImageUniforms);
            builder.AppendLine(string.Format("Max Combined Image Uniforms: {0}", maxImageUniforms[0]));
