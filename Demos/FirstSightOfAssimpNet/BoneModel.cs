    ...
    class BoneModel : IBufferSource
    {
        ...
        public IEnumerable<VertexBuffer> GetVertexAttribute(string bufferName)
        {
            if (strPosition == bufferName)
            {
                if (this.positionBuffer == null)
                {
                    AllBones allBones = this.container.GetAllBones();
                    int boneCount = allBones.boneInfos.Length;
                    var poistions = new vec3[boneCount];
                    this.positionBuffer = poistions.GenVertexBuffer(VBOConfig.Vec3, BufferUsage.StaticDraw);
                }
        ...
        public IEnumerable<IDrawCommand> GetDrawCommand()
        {
            if (this.drawCommand == null)
            {
                AllBones allBones = this.container.GetAllBones();
                int boneCount = allBones.boneInfos.Length;
                this.drawCommand = new DrawArraysCmd(DrawMode.Lines, boneCount);
            }
