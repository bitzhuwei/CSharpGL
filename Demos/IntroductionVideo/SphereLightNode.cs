...
namespace IntroductionVideo {
    partial class SphereLightNode : ModernNode, IBlinnPhong {
        public static SphereLightNode Create(IBufferSource model, string position, string normal, vec3 size) {
            ...
            var node = new SphereLightNode(model, ambientBuilder, blinnPhongBuilder);
            node.Initialize();
            node.ModelSize = size;
        ...
        private SphereLightNode(IBufferSource model, params RenderMethodBuilder[] builders)
            : base(model, builders) {
