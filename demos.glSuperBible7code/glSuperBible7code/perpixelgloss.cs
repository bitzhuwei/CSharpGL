...
namespace demos.glSuperBible7code {
    ...
    public unsafe class perpixelgloss : _glSuperBible7code {
        ~perpixelgloss() {
        ...
        public perpixelgloss(Form mainForm, int width, int height, GL gl)
        ...
        public override void init(CSharpGL.GL gl) {
            {
                var vsCodeFile = "media/shaders/perpixelgloss.vert";
                var fsCodeFile = "media/shaders/perpixelgloss.frag";
            ...
            }
            {
                ...
                tex_envmap = sb7ktx.load("media/textures/mountains3d.ktx");

                gl.glTexParameteri(GL.GL_TEXTURE_3D, GL.GL_TEXTURE_MIN_FILTER, (int)GL.GL_LINEAR);
                gl.glTexParameteri(GL.GL_TEXTURE_3D, GL.GL_TEXTURE_MAG_FILTER, (int)GL.GL_LINEAR);
                gl.glTexParameteri(GL.GL_TEXTURE_3D, GL.GL_TEXTURE_WRAP_S, (int)GL.GL_CLAMP_TO_EDGE);
                gl.glTexParameteri(GL.GL_TEXTURE_3D, GL.GL_TEXTURE_WRAP_T, (int)GL.GL_CLAMP_TO_EDGE);
                gl.glTexParameteri(GL.GL_TEXTURE_3D, GL.GL_TEXTURE_WRAP_R, (int)GL.GL_CLAMP_TO_EDGE);

                gl.glActiveTexture(GL.GL_TEXTURE1);
                tex_glossmap = sb7ktx.load("media/textures/pattern1.ktx");
        ...
        public override void display(CSharpGL.GL gl) {
            ...
            gl.glActiveTexture(GL.GL_TEXTURE0);
            gl.glBindTexture(GL.GL_TEXTURE_3D, tex_envmap);

            //layout (binding = 1) uniform sampler2D tex_glossmap;
            gl.glActiveTexture(GL.GL_TEXTURE1);
            gl.glBindTexture(GL.GL_TEXTURE_2D, tex_glossmap);

            gl.glUseProgram(render_prog);

            mat4 mv_matrix = glm.translate(0.0f, 0.0f, -3.0f) *
                glm.rotate(currentTime * 13.75f, 0.0f, 1.0f, 0.0f) *
                glm.rotate(currentTime * 7.75f, 0.0f, 0.0f, 1.0f) *
                glm.rotate(currentTime * 15.3f, 1.0f, 0.0f, 0.0f);
