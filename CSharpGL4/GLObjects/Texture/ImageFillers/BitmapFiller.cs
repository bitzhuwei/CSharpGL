...
namespace CSharpGL
{
    ...
    public class BitmapFiller : ImageFiller
    {
        ...
        /// <param name="internalformat">GL.GL_RGBA etc.</param>
        ...
        public override void Fill()
        {
            ...
            if (target == TextureTarget.Texture1D)
            {
                ...
                for (int level = 0; level <= this.maxLevel; level++)
                {
                    ...
                    GL.Instance.TexImage1D((uint)target, level, this.internalformat, bitmap.Width, 0, this.format, this.type, bitmapData.Scan0);
                    //  Unlock the image.
            ...
            else if (target == TextureTarget.Texture2D)
            {
                ...
                for (int level = 0; level <= this.maxLevel; level++)
                {
                    ...
                    BitmapData bitmapData = bmp.LockBits(new Rectangle(0, 0, bmp.Width, bmp.Height),
                        ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
                    GL.Instance.TexImage2D((uint)target, level, this.internalformat, bmp.Width, bmp.Height, 0, this.format, this.type, bitmapData.Scan0);
                    //  Unlock the image.
                    bmp.UnlockBits(bitmapData);
                }
