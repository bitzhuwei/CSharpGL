    ...
    class QuadModel : IBufferSource {
        ...
        public IEnumerable<VertexBuffer> GetVertexAttribute(string bufferName) {
            if (positions == bufferName) {
                if (this.positionBuffer == null) {
                    this.positionBuffer = quadVerts.GenVertexBuffer(VBOConfig.Vec2, GLBuffer.Usage.StaticDraw);
                }
        ...
        public IEnumerable<IDrawCommand> GetDrawCommand() {
            if (this.drawCmd == null) {
                IndexBuffer buffer = quadIndices.GenIndexBuffer(IndexBuffer.ElementType.UShort, GLBuffer.Usage.StaticDraw);
