    ...
    class ManyCubesModel : IBufferSource {
        ...
        public IEnumerable<VertexBuffer> GetVertexAttribute(string bufferName) {
            if (bufferName == strPosition) {
                if (this.positionBuffers == null) {
                    ...
                    this.positionBuffers = positions.GenVertexBuffers(VBOConfig.Vec3, GLBuffer.Usage.StaticDraw, this.blockSize);
                }
                ...
                if (this.colorBuffers == null) {
                    ...
                    this.colorBuffers = colors.GenVertexBuffers(VBOConfig.Vec3, GLBuffer.Usage.StaticDraw, this.blockSize);
                }
        ...
        public IEnumerable<IDrawCommand> GetDrawCommand() {
            if (this.drawCmds == null) {
                ...
                IndexBuffer[] buffers = indexes.GenIndexBuffers(IndexBuffer.ElementType.UInt, GLBuffer.Usage.StaticDraw, this.blockSize);
                var cmds = new IDrawCommand[buffers.Length];
                for (int i = 0; i < buffers.Length; i++) {
                    cmds[i] = new DrawElementsCmd(buffers[i], CSharpGL.DrawMode.QuadStrip, uint.MaxValue);
                }
                this.drawCmds = cmds;
            }
