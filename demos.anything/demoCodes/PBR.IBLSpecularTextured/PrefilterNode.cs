        ...
        protected unsafe override void DoInitialize() {
            ...
            for (int mip = 0; mip < maxMipLevels; ++mip) {
                ...
                for (uint i = 0; i < 6; ++i) {
                    ...
                    //int level = 0;
                    captureFBO.Bind();
                    captureFBO.Attach(Framebuffer.Target.Framebuffer, location, face, this.prefilterMap, mip);
                    captureFBO.CheckCompleteness();
                    captureFBO.Unbind();
                    ...
                    var glBmp = this.prefilterMap.GetImage(face, mipWidth, mipHeight, mip);
                    var bitmap = new Bitmap(mipWidth, mipHeight, mipWidth * glBmp.PixelBytes,
                        System.Drawing.Imaging.PixelFormat.Format32bppArgb, glBmp.Scan0);
                    bitmap.Save(string.Format("prefilter.{0}.mip{1}.png", face, mip));
                }
