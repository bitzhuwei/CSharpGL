    ...
    partial class CubemapNode : ModernNode, IRenderable {
        ...
        protected unsafe override void DoInitialize() {
            ...
            for (uint i = 0; i < 6; ++i) {
                ...
                int level = 0;
                captureFBO.Bind();
                captureFBO.Attach(Framebuffer.Target.Framebuffer, location, face, this.environmentMap, level);
                captureFBO.CheckCompleteness();
                captureFBO.Unbind();
                ...
                var glBmp = this.environmentMap.GetImage(face, 512, 512);
                var bitmap = new Bitmap(512, 512, 512 * glBmp.PixelBytes,
                    System.Drawing.Imaging.PixelFormat.Format32bppArgb, glBmp.Scan0);
                bitmap.Save(string.Format("texEnvCubemap.{0}.mip{1}.png", face, 0));
            }
            viewportSwitch.Off();
            captureFBO.Dispose();
