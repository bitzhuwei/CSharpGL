    ...
    partial class IrradianceNode : ModernNode, IRenderable {
        ...
        protected unsafe override void DoInitialize() {
            ...
            for (uint i = 0; i < 6; ++i) {
                ...
                int level = 0;
                captureFBO.Bind();
                captureFBO.Attach(Framebuffer.Target.Framebuffer, location, face, this.texIrradianceMap, level);
                captureFBO.CheckCompleteness();
                captureFBO.Unbind();
                ...
                var glBmp = this.texIrradianceMap.GetImage(face, length, length);
                var bitmap = new Bitmap(length, length, length * glBmp.PixelBytes,
                    System.Drawing.Imaging.PixelFormat.Format32bppArgb, glBmp.Scan0);
                bitmap.Save(string.Format("irradianceMap.{0}.mip{1}.png", face, 0));
            }
            viewportSwitch.Off();
            captureFBO.Dispose();
        }
