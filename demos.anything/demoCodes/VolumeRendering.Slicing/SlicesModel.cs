    ...
    class SlicesModel : IBufferSource {
        ...
        public IEnumerable<VertexBuffer> GetVertexAttribute(string bufferName) {
            if (bufferName == position) {
                if (this.slicesBuffer == null) {
                    this.slicesBuffer = this.vTextureSlices.GenVertexBuffer(VBOConfig.Vec3, GLBuffer.Usage.DynamicDraw);
                }
        ...
        public IEnumerable<IDrawCommand> GetDrawCommand() {
            if (this.drawCmd == null) {
                this.drawCmd = new DrawArraysCmd(CSharpGL.DrawMode.Triangles, this.vTextureSlices.Length);
            }
