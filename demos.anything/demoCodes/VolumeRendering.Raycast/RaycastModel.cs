    ...
    internal class BoundingBoxModel : IBufferSource {
        ...
        public IEnumerable<VertexBuffer> GetVertexAttribute(string bufferName) {
            if (bufferName == strPosition) {
                if (this.positionBuffer == null) {
                    ...
                    //VertexBuffer buffer = VertexBuffer.Create(typeof(float), length, VBOConfig.Vec3, varNameInShader, GLBuffer.BufferUsage.StaticDraw);
                    //unsafe
                    //{
        ...
        public IEnumerable<IDrawCommand> GetDrawCommand() {
            if (drawCmd == null) {
                ...
                //IndexBuffer buffer = CSharpGL.Buffer.Create(IndexElementType.UInt, length, GLBuffer.BufferUsage.StaticDraw);
                //unsafe
                //{
                //    IntPtr pointer = buffer.MapBuffer(MapBufferAccess.WriteOnly);
                ...
                //    buffer.UnmapBuffer();
                //}
                IndexBuffer buffer = indexes.GenIndexBuffer(IndexBuffer.ElementType.UInt, GLBuffer.Usage.StaticDraw);
                this.drawCmd = new DrawElementsCmd(buffer, CSharpGL.DrawMode.Quads);
            }
