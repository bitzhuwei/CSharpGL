...
namespace CSharpGL {
    public unsafe partial class vgl {
        public static void vglLoadDDS(string filename, ref vglImageData image) {
            ...
            //var data = new UnmanagedArray<byte>(elementCount: image.totalDataSize);
            //var data = new byte[image.totalDataSize];
            //var pointer = GCHandle.Alloc(data, GCHandleType.Pinned);
            var data = Marshal.AllocHGlobal(image.totalDataSize);
            if (image.mip == null) { image.mip = new vglImageMipData[vermilion.MAX_TEXTURE_MIPS]; }
            image.mip[0].data = data;//pointer.AddrOfPinnedObject();// data.Header;
            //pointer.Free();
