...
namespace CSharpGL {
    ...
    /// </summary>
    internal class HalfSphereModel {
        ...
        /// <returns></returns>
        internal HalfSphereModel(float radius = 1.0f, int latitudeParts = 10, int longitudeParts = 40) {
            ...
            for (int i = 0; i < latitudeParts + 1; i++) {
                double theta = (latitudeParts - i * 2) * Math.PI / 2 / latitudeParts;
                double y = (i <= latitudeParts / 2) ? radius * Math.Sin(theta) : 0;
                for (int j = 0; j < longitudeParts + 1; j++) {
                    ...
                    this.normals[index] = (i <= latitudeParts / 2) ? position.normalize() : new vec3(0, -1, 0);
