            ...
            for (int i = 0; i < result.Length; i++) {
                ...
                foreach (var edge in edges) {
                    ...
                    if (dict.TryGetValue(edge, out sharedFaces)) {
                        ...
                        if (otherFaceIndex == faces.Length) { adjacentVertexes.Add(edge.vertexId1); }
                        else {
                            Face otherFace = faces[otherFaceIndex];
                            if (otherFace.vertexId1 != edge.vertexId1
                                && otherFace.vertexId1 != edge.vertexId2) { adjacentVertexes.Add(otherFace.vertexId1); }
                            else if (otherFace.vertexId2 != edge.vertexId1
                                && otherFace.vertexId2 != edge.vertexId2) { adjacentVertexes.Add(otherFace.vertexId2); }
                            else if (otherFace.vertexId3 != edge.vertexId1
                                && otherFace.vertexId3 != edge.vertexId2) { adjacentVertexes.Add(otherFace.vertexId3); }
                            else { throw new Exception(string.Format("all 3 vertexes are on the edge?!")); }
                        }
                    }
                    else {
            ...
            for (int i = 0; i < faces.Length; i++) {
                ...
                foreach (var edge in edges) {
                    ...
                    if (result.TryGetValue(edge, out sharedFaces)) {
                        if (sharedFaces.face2 < faces.Length) { throw new Exception(string.Format("More than 2 faces share an edge! [{0}]", i)); }
