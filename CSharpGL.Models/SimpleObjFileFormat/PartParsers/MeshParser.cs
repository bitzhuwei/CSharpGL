...
namespace CSharpGL {
    public unsafe class MeshParser : ObjParserBase {
        ...
        public override void Parse(ObjVNFContext context) {
            ...
            if (vertexIndex != context.vertexCount) { throw new Exception(string.Format("v: [{0}] not equals to [{1}] in MeshParser!", vertexIndex, context.vertexCount)); }
            if (normalIndex != context.normalCount) { throw new Exception(string.Format("vn: [{0}] not equals to [{1}] in MeshParser!", normalIndex, context.normalCount)); }
            if (texCoordIndex != context.texCoordCount) { throw new Exception(string.Format("vt: [{0}] not equals to [{1}] in MeshParser!", texCoordIndex, context.texCoordCount)); }
            if (faceIndex != context.faceCount) { throw new Exception(string.Format("f: [{0}] not equals to [{1}] in MeshParser!", vertexIndex, context.vertexCount)); }
            if (faceIndex > 0) {
        ...
        private void ParseFaceUnit(string unit, out uint vertexIndex, out uint normalIndex, out uint texCoordIndex) {
            ...
            if (parts.Length == 1) {
                ...
                vertexIndex = uint.Parse(parts[0]);
                if (parts[1].Trim() != string.Empty) { texCoordIndex = uint.Parse(parts[1]); }
                ...
                if (parts[2].Trim() != string.Empty) { normalIndex = uint.Parse(parts[2]); }
                else { normalIndex = 0; }
            }
            else {
