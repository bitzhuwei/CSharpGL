...
namespace SoftGLImpl {
    partial class SoftGL {
        private static unsafe ConcurrentBag<FragmentCodeBase[]> LinearInterpolationAndFragmentShaderStage4QuadStrip(
            ...
            for (int indexID = indices.ToInt32() / elementBytes, c = 0; c < count - 3 && indexID < indexCount - 3; indexID += 2, c += 2) {
                ...
                var fsInstances1 = FindFragmentsInTriangle_V3(//fragCoord0, fragCoord1, fragCoord2,
                    fragCoords[0], fragCoords[1], fragCoords[2],
                    endpoints[0], endpoints[1], endpoints[2], program);
                result.Add(fsInstances1);

                var fsInstances2 = FindFragmentsInTriangle_V3(//fragCoord1, fragCoord3, fragCoord2,
                    fragCoords[1], fragCoords[3], fragCoords[2],
                    endpoints[1], endpoints[3], endpoints[2], program);
