    ...
    public class FormatContext {
        ...
        public LastComment PrintCommentsBefore(int end, BlankConfig config, TextWriter writer) {
            ...
            while (0 <= start) {
                ...
                if (false
                 // || token.type == CompilerPreDirective.st.blockComment块
                 // || token.type == CompilerPreDirective.st.inlineComment行
                 ) {
                    start--;
                }
                else { break; }
        ...
        public LastComment PrintCommentsBetween(int start, int end, BlankConfig config, TextWriter writer) {
            ...
            if (start <= end) {
                ...
                for (int i = start; i <= end; i++) {
                    var token = tokens[i];
                    if (false) { }
                    //block comment is not supported for this grammar
                    //else if (token.type == CompilerPreDirective.st.blockComment块) {
                    //    PrintBlanksBefore(token, config, writer);
                    //    writer.Write(token.value);
                    //    lastComment = LastComment.BlockComment;
                    //}
                    else if (token.type == CompilerPreDirective.st.inlineComment行) {
                        PrintBlanksBefore(token, config, writer);
        ...
        /// <param name="config">if null, it means to print this <paramref name="token"/> directly(without any pre-space-char)</param>
        /// <param name="writer"></param>
        ...
        /// </summary>
        /// <param name="token"></param>
        /// <param name="config">if null, it means to print nothing</param>
        /// <param name="writer"></param>
