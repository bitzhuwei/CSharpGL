...
namespace bitzhuwei.GLSLFormat {
    partial class CompilerGLSL {
        ...
        private static void AcceptToken(LexicalContext context, int Vt) {
            ...
            var token = context.analyzingToken.Dump(context.stArray, end);
            context.result.Add(token);
            // 跳过注释 skip block/inline comment
            if (context.analyzingToken.kind != st.blockComment块
             && context.analyzingToken.kind != st.inlineComment行) {
        ...
        private static void AcceptToken3(LexicalContext context, params IfVt[] ifVts) {
            ...
            #region user-added part
            const string key = "type_name"; var hadThisTypeName = false;
            {
                if (context.tagDict.TryGetValue(key, out var type_nameList)) {
                    if (type_nameList is List<string> list && list.Contains(context.analyzingToken.value)) {
                        context.analyzingToken.kind = st.type_name;
                        typeSet = true;
                        hadThisTypeName = true;
                    }
                }
            }
            #endregion user-added part
            ...
            #region user-added part
            if (!hadThisTypeName && context.analyzingToken.kind == st.type_name) {
                if (!context.tagDict.TryGetValue(key, out var type_nameList)) {
                    type_nameList = new List<string>();
                    context.tagDict.Add(key, type_nameList);
                }
                if (type_nameList is List<string> list) {
