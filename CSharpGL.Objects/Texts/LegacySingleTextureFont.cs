...
{
    class LegacySingleTextureFont : IDisposable
    {
        Dictionary<char, GlyphRectangle> glyphRectDict = new Dictionary<char, GlyphRectangle>();
        ...
        public LegacySingleTextureFont(string font, int size)
        {
            // 保存字体大小，在渲染时用
            fontHeight = size;
            ...
            // 为所有可打印的字符的创建纹理
            const int textureCount = 1;// char.MaxValue;
        ...
        public void Compile_Character(FreeTypeFace face, int c, int fontHeight)
        {
            ...
            if (size <= 0)
            {
                ...
                if (c == 32)
                {
                    ...
                    GL.Translatef(fontHeight >> 1, 0, 0);
                    extent_x[c] = fontHeight >> 1;
                    GL.EndList();
                }
                return;
