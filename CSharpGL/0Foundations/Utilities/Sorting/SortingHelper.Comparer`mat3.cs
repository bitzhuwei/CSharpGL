...
namespace CSharpGL
{
    ...
    public static partial class SortingHelper
    {
        ...
        public static void Sort(this UnmanagedArray<mat3> array, Func<mat3, mat3, int> comparer)
        {
        ...
        /// If you want descending sort, make it returns -1 when <paramref name="array"/>[left] &lt; <paramref name="array"/>[right].
        /// <para>Otherwise, make it returns -1 when <paramref name="array"/>[left] &gt; <paramref name="array"/>[right].</para></param>
        public static void Sort(this UnmanagedArray<mat3> array, int start, int length, Func<mat3, mat3, int> comparer)
        {
            QuickSort(array, start, start + length - 1, comparer);
        }

        private static void QuickSort(UnmanagedArray<mat3> array, int start, int end, Func<mat3, mat3, int> comparer)
        {
            if (start >= end) { return; }
        ...
        private static void QuickSort(UnmanagedArray<mat3> array, Func<mat3, mat3, int> comparer, Stack<int> stack)
        {
            while (stack.Count > 0)
            {
                int start = stack.Pop();
        ...
        private static unsafe int QuickSortPartion(UnmanagedArray<mat3> array, int start, int end, Func<mat3, mat3, int> comparer)
        {
            var pointer = (mat3*)array.Header.ToPointer();
            mat3 pivot, startValue, endValue;
            pivot = pointer[start];
            while (start < end)
            {
