...
namespace CSharpGL {
    public abstract unsafe partial class GLBuffer {
        ...
        public bool ClearBufferData(vec3 data, bool autoBind = true) {
            ...
            vec3* pointer = &data;
            bool result = ClearBufferData(GL.GL_RGB32F, GL.GL_RGB, GL.GL_FLOAT, (IntPtr)pointer, autoBind);
        ...
        public bool ClearBufferData(uint internalFormat, uint format, uint type, IntPtr data, bool autoBind = true) {
            ...
            //if (glClearBufferData == null) { glClearBufferData = gl.glGetDelegateFor("glClearBufferData", GLDelegates.typeof_void_uint_uint_uint_uint_IntPtr) as GLDelegates.void_uint_uint_uint_uint_IntPtr; }

            var gl = GL.current; if (gl == null) { return false; }
            var glClearBufferData = gl.glClearBufferData;
            bool result = (glClearBufferData != null);
            if (result) {
                if (autoBind) {
                    gl.glBindBuffer((GLenum)this.target, this.bufferId);
                }
                glClearBufferData((GLenum)this.target, internalFormat, format, type, data);
                if (autoBind) {
        ...
        /// <param name="autoBind">Automatically call glBindBuffer() inside this method.</param>
        /// <returns></returns>
        public bool ClearBufferSubData(int offset, int size, vec3 data, bool autoBind = true) {
            //var array = new vec3[] { data };
            //GCHandle pinned = GCHandle.Alloc(array, GCHandleType.Pinned);
            //IntPtr header = pinned.AddrOfPinnedObject();
            //// same result with: IntPtr header = Marshal.UnsafeAddrOfPinnedArrayElement(array, 0);
            ...
            //pinned.Free();
            vec3* pointer = &data;
            bool result = ClearBufferSubData(GL.GL_RGB32F, new IntPtr(offset), size, GL.GL_RGB, GL.GL_FLOAT, (IntPtr)pointer, autoBind);
        ...
        ///// <param name="autoBind">Automatically call glBindBuffer() inside this method.</param>
        ///// <returns></returns>
        //public bool ClearBufferSubData(uint internalFormat, IntPtr offset, int size, uint format, uint type, UnmanagedArrayBase data, bool autoBind = true) {
        //    if (data == null || data.Header == IntPtr.Zero) { throw new ArgumentNullException("data"); }
        ...
        /// <param name="autoBind">Automatically call glBindBuffer() inside this method.</param>
        /// <returns></returns>
        public bool ClearBufferSubData(uint internalFormat, IntPtr offset, int size, uint format, uint type, IntPtr data, bool autoBind = true) {
            if (data == IntPtr.Zero) { throw new ArgumentNullException("data"); }

            //if (glClearBufferSubData == null) { glClearBufferSubData = gl.glGetDelegateFor("glClearBufferSubData", GLDelegates.typeof_void_uint_uint_IntPtr_uint_uint_uint_IntPtr) as GLDelegates.void_uint_uint_IntPtr_uint_uint_uint_IntPtr; }

            var gl = GL.current; if (gl == null) { return false; }
            var glClearBufferSubData = gl.glClearBufferSubData;
            bool result = (glClearBufferSubData != null);
            if (result) {
                if (autoBind) {
                    gl.glBindBuffer((uint)this.target, this.bufferId);
                }
                gl.glClearBufferSubData((uint)this.target, internalFormat, offset, size, format, type, data);
                if (autoBind) {
                    gl.glBindBuffer((uint)this.target, 0);
                }
            }
