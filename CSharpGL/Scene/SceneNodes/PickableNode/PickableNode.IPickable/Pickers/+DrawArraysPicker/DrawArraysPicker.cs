    ...
    partial class DrawArraysPicker : PickerBase {
        ...
        public override PickedGeometry? GetPickedGeometry(PickingEventArgs arg, uint stageVertexId, uint baseId) {
            ...
            if ((pickingType & PickingGeometryTypes.Point) == PickingGeometryTypes.Point) {
                ...
                if (typeOfMode == GeometryType.Point) { return PickWhateverItIs(arg, stageVertexId, singleNodeVertexId, mode, typeOfMode); }
                else if (typeOfMode == GeometryType.Line) {
                    if (this.OnPrimitiveTest(singleNodeVertexId, mode)) { return PickPoint(arg, stageVertexId, singleNodeVertexId); }
                    else { return null; }
                }
                else {
                    var searcher = GetPointSearcher(mode);
                    if (searcher != null)// point is from triangle, quad or polygon
                    { return SearchPoint(arg, stageVertexId, singleNodeVertexId, searcher); }
                    else { throw new Exception(string.Format("Lack of searcher for [{0}]", mode)); }
                }
            }
            else if ((pickingType & PickingGeometryTypes.Line) == PickingGeometryTypes.Line) {
                DrawMode mode = this.DrawCommand.Mode;
                GeometryType typeOfMode = mode.ToGeometryType();
                if (pickingType.Contains(typeOfMode)) { return PickWhateverItIs(arg, stageVertexId, singleNodeVertexId, mode, typeOfMode); }
                else {
                    var searcher = GetLineSearcher(mode);
        ...
        private PickedGeometry PickWhateverItIs(PickingEventArgs arg, uint stageVertexId, uint singleNodeVertexId, DrawMode mode, GeometryType typeOfMode) {
            ...
            if (vertexCount == -1) { vertexCount = (from item in this.Node.PickingRenderMethod.PositionBuffers select item.count).Sum(); }
            ...
            if (singleNodeVertexId == 0 && vertexCount == 2) {
                ...
                for (uint i = 0; i < vertexCount; i++) { vertexIds[i] = singleNodeVertexId + i; }
        ...
        private void PickingLastLineInLineLoop(out uint[] vertexIds, out vec3[] positions) {
            ...
            int sum = (from item in buffers select item.count).Sum();
