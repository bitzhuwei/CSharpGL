...
namespace CSharpGL
{
    internal class DrawArraysPointInPolygonSearcher : DrawArraysPointSearcher
    {
            ...
            uint flatColorVertexId, DrawArraysPicker picker)
        {
            ...
            var drawCmd = new DrawArraysCmd(DrawMode.Points, cmd.FirstVertex, cmd.RenderingVertexCount, cmd.FrameCount);
            picker.Node.Render4InnerPicking(arg, IndexAccessMode.ByFrame, drawCmd);
            uint id = ColorCodedPicking.ReadStageVertexId(arg.X, arg.Y);

            if (cmd.FirstVertex <= id && id < cmd.FirstVertex + cmd.RenderingVertexCount)
            { return id; }
            else
