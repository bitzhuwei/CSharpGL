    ...
    partial class ComputeShaderNode : PickableNode, IRenderable {
        ...
        public static ComputeShaderNode Create() {
            ...
            RenderMethodBuilder fuzzy, inverseColor, mosaic, render;
            {
                ...
                var program = GLProgram.Create((fuzzyComp, Shader.Kind.comp)); Debug.Assert(program != null);
                var map = new AttributeMap();
                fuzzy = new RenderMethodBuilder(program, map);
            }
            {
                //var cs = new CSharpGL.ComputeShader(inverseColorComp);
                //var provider = new ShaderArray(cs);
                var program = GLProgram.Create((inverseColorComp, Shader.Kind.comp)); Debug.Assert(program != null);
                var map = new AttributeMap();
                inverseColor = new RenderMethodBuilder(program, map);
            }
            {
                //var cs = new CSharpGL.ComputeShader(mosaicComp);
                //var provider = new ShaderArray(cs);
                var program = GLProgram.Create((mosaicComp, Shader.Kind.comp)); Debug.Assert(program != null);
                var map = new AttributeMap();
                mosaic = new RenderMethodBuilder(program, map);
            }
            {
        ...
        public unsafe void RenderBeforeChildren(RenderEventArgs arg) {
            ...
            uint inputUnit = 0, outputUnit = 1;
            {
                ...
                computeProgram.Bind();
                gl.glBindImageTexture(inputUnit, this.inTexture.id, 0, false, 0, GL.GL_READ_WRITE, GL.GL_RGBA32F);
                gl.glBindImageTexture(outputUnit, this.outTexture.id, 0, false, 0, GL.GL_READ_WRITE, GL.GL_RGBA32F);
                // Dispatch
                gl.glDispatchCompute(config.groupXCount, config.gropuYCount, 1);
